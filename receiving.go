...
func (c *Client) ReceiveNotification(ctx context.Context) (map[string]interface{}, error) {
	var result map[string]interface{}
	err := c.request(ctx, "GET", c.basePath+"/receiveNotification", nil, &result)
	return result, err
}

// DeleteNotification deletes an incoming notification from the notification queue
func (c *Client) DeleteNotification(ctx context.Context, receiptID int64) (*DeleteNotificationResponse, error) {
	var result DeleteNotificationResponse
	err := c.request(ctx, "DELETE", fmt.Sprintf("%s/deleteNotification/%d", c.basePath, receiptID), nil, &result)
	return &result, err
}

// GetChatHistory returns the message history for a specified chat
func (c *Client) GetChatHistory(ctx context.Context, params GetChatHistoryParams) ([]map[string]interface{}, error) {
	var result []map[string]interface{}
	err := c.request(ctx, "POST", c.basePath+"/getChatHistory", params, &result)
	return result, err
}
